...
| **Auto-scroll ke field yang difokuskan belum ada.** Tahap 2.5 hanya menambah ruang bawah ScrollView sebesar lift keyboard; ruang ≠ gulungan. ScrollView core RN tidak menggulung sendiri ke input yang di-tap saat window tidak menyusut, jadi field di bawah garis keyboard masih bisa tertutup sampai user menggulung manual. | Unit terpisah, sesudah profil |
| **Gerakan `stickyFooter` menyentak di Android.** `keyboardDidShow` baru tiba setelah animasi keyboard selesai, jadi footer melompat, tidak meluncur. `keyboardWillShow` tidak pernah dikirim Android, jadi tidak ada obat lewat core API. Kosmetik. | Unit terpisah / terima apa adanya |
| **iOS belum pernah diuji sama sekali untuk SELURUH perombakan profil** (Tahap 1–3B), bukan cuma bagian keyboard. Yang nol bukti perilaku: jalur `keyboardWillShow`/`keyboardWillHide` dan rumus lift yang basis `screenY`-nya diverifikasi dari sumber RN **Android**; `gestureEnabled:false` pada rute `profile-edit`; toggle `secureTextEntry` di layar password; serta tampilan `ConfirmDialog` dengan `cancelTone`. Semua verifikasi sejauh ini Android + Expo Go. | Saat ada device/simulator iOS |
| **`tokens.layout.fieldHeight` (54) vs `controlHeight` (56)** — selisih 2px yang kemungkinan tidak disengaja. Jangan disisipkan di unit lain; rekonsiliasi butuh satu pass tersendiri dengan verifikasi visual karena menyentuh semua form. | Unit terpisah, dengan verifikasi device |
| **Route guard bisa menendang keluar tanpa lewat `useUnsavedChangesGuard`.** Ketiga `_layout.tsx` memanggil `refresh()` tiap fokus lalu `router.replace(targetRoute)` kalau status keanggotaan berubah. Kalau owner menonaktifkan akses pekerja saat pekerja itu sedang mengisi Edit Profil, layar berpindah paksa dan perubahannya hilang diam-diam — penjaga hanya melihat chevron dan back fisik, bukan `replace` yang datang dari layout. | Unit yang menyentuh route guard |
| **Verifikasi password lama memakai sign-in sungguhan.** `verifyCurrentPassword` memanggil `signInWithPassword` di client kedua, jadi tiap salah ketik adalah percobaan login nyata dan ikut memakan kuota rate limit auth akun itu — berpotensi ikut memblokir **login** untuk sementara, bukan cuma layar ubah password. Konsekuensi melekat dari verifikasi sisi klien; hilang hanya kalau pindah ke penegakan sisi server. | Kalau ubah password dipindah ke RPC/Edge Function |
| **Sesi bayangan meninggalkan refresh token menganggur.** `signOut({ scope: 'local' })` hanya membersihkan memori client kedua; token hasil verifikasi tetap hidup di sisi Supabase sampai kedaluwarsa. `scope: 'global'` bukan pilihan — itu mencabut sesi utama user juga. Jadi tiap ganti password menyisakan satu refresh token menganggur. Bukan kebocoran kredensial. | Kalau ubah password dipindah ke RPC/Edge Function |
